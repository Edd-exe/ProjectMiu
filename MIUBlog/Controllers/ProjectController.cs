...
namespace MIUBlog.WebUI.Controllers
{
    public class ProjectController : Controller
    {
        ...
        public async Task<IActionResult> ProjectApplication(int id, string applicationUserId)
        {
        ...
        public async Task<IActionResult> Details(int id, Project blg)
        {
            ...
            if (User.Identity.IsAuthenticated)
            {
                ...
                ApplicationUser user = await _userManager.FindByNameAsync(User.Identity.Name);
                projectUser = _projectUserService.GetByProjectIdAndUserId(projectId, user.Id);
            }
            var project = _projectService.Get(projectId);
            var comments = _commentService.GetAll()
                .Include(i => i.ApplicationUser)
                .Where(i => i.Project.Id == projectId && i.isApproved == true);
            List<ProjectUser> projectUsers = _projectUserService.GetByProjectId(projectId).Where(p=>p.isApproved==true).ToList();
